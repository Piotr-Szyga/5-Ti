...
function getParams(&$form){
	...
	$form['h'] = isset($_REQUEST['h']) ? $_REQUEST['h'] : null;
}
...
function validate(&$form,&$infos,&$msgs,&$hide_intro){
	...
	if ( ! (isset($form['x']) && isset($form['y']) && isset($form['h'])))	return false;	
	...
	if ( $form['h'] == "") $msgs [] = 'Nie podano liczby 3';
	...
	if ( count($msgs)==0 ) {
		...
		if (! is_numeric( $form['h'] )) $msgs [] = 'Trzecia wartość nie jest liczbą';
	}
...
function process(&$form,&$infos,&$msgs,&$result){
	...
	$form['h'] = floatval($form['h']);
	
	//wykonanie operacji
	
				$result = ($form['x'] + $form['y']) * $form['h'] / 2;
	}
